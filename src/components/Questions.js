...
const Questions = () => {
  ...
  useEffect(() => {
    ...
    const checkPreviousSubmission = async () => {
      try {
        const response = await fetch(
          `/api/auth/check-submission?studentId=${studentData.rollno}&semester=${studentData.semester}&year=${studentData.year}`,
          {
            headers: {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const response = await fetch(
          `http://117.213.202.136:5000/api/auth/questions?branch=${encodeURIComponent(studentData.branch)}&sem=${encodeURIComponent(studentData.semester)}&year=${encodeURIComponent(studentData.year)}`,
          {
            headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch('http://117.213.202.136:5000/api/auth/submitAnswers', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
